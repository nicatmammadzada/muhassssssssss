...
//             ->where('is_active', 1)->get();
    ...
    public function filter(Request $request)
    {
        ...
        // $clients = Client::
        // with(['orderList' => function ($query) use($pr) {
        //     $query
        //         ->groupBy('client_id')
        //          ->with('orders.product.category')
        //          ->where('orderList.orders.product_id',$pr)
        //         ;
        // }])
        //     ->orderBy('id', 'desc')
        //     ->where('is_active', 1)->get();

// $query->whereHas('orders', $constraint)
//                  ->with([$relation => $constraint])

$clients=Client::with('orderList')->
    WhereHas('orderList.orders', function($query) use ($pr){
    $query->where('product_id', $pr);
})->
get();
...
//           $clients = Client::
//         with(['orderList.orders' => function ($query) use ($pr) {
//             $query
//                 ->groupBy('client_id')
//                 ->where('product_id',$pr)
//                 ->with('product.category')
