...
@section('content')
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title">Order Show</h5>
            <div class="heading-elements">
                <a href="{{route('admin.cv.index')}}"><span class="label label-success">BACK TO LIST</span></a>
            </div>
        </div>
            ...
            @if($orders->count()>0)
                @foreach($orders as $key=>$order)
                    ...
                    <td></td>
                        ...
                        <td><a href="{{route('admin.product.edit',$order->product_id)}}">{{$order->product->name ?? ''}}</a></td>
                        <td><a href="{{route('admin.product.edit',$order->product_id)}}">{{$order->count ?? ''}}</a></td>

                        <td><a href="{{route('admin.product.edit',$order->product_id)}}">{{$order->price ?? ''}} </a></td>
                        <td><a href="{{route('admin.product.edit',$order->product_id)}}">{{$order->total ?? ''}} </a></td>
...
{{--                                        <li><a href="{{route('admin.product.edit',$cv->product_id)}}"><i class="icon-database-edit2"></i> Yenilə</a></li>--}}
{{--                                        <li><a  onclick='checkDeleteConfrim("{{route('admin.product.destroy',$cv->id)}}")'><i class="icon-database-remove"></i> Sil</a></li>--}}
{{--                                    </ul>--}}
